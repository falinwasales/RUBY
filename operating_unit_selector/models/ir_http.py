...
class IrHttp(models.AbstractModel):
    ...
    def session_info(self):
        ...
        if self.env.user.has_group('base.group_user'):
            ...
            result.update({
                "user_operating_units": {'current_operating_unit': (user.default_operating_unit_id.id, user.default_operating_unit_id.name), 'assigned_operating_units': [(aou.id, aou.name) for aou in user.assigned_operating_unit_ids], 'operating_units': [(ou.id, ou.name) for ou in user.operating_unit_ids]},
